...
Names: limited to 64 characters on unix, 15 character on Win
...
Resizing a running VM will reboot the VM

Computed priced per hour but billed per minute
...
Replicates a workloads from a primary site to a secondary location. (Using Azure). Also meant for failover drills.

Can include Powershell scripts, Azure Automation run books, manual sets. 

### Azure Backup

Can backup files and folders, snapshots, Microsoft server workloads (SQL server)
## Azure App Service
...
Types:
  ...
  * internal:
    ...
    * cross prem virtual network
    * multi-tiered applications

Operates at Transport Layer (Layer 4): destination, TCP or UDP, port

Azure Application Gateway manages traffic on Layer 7.

Implements automatic reconfiguration to redistribute load across altered number of instances

Provides stickiness only within a traffic session
...
Alternatives:
  ...
  * Azure Front Door: global load balancing and site acceleration service, layer 7 capabilities including path routing, fast failover, firewalls
...
TLS termination
Security filtering
Round robin to process load balancing requests
Support for HTTP, HTTPs, HTTP/2, Websockers
  
Parts:
  * front end IP
  * listeners <—- can be basic or multi-site. Basic listeners routes based on path; multi-site listener routes requests using hostname.
  * Routing rules. Specify if/how traffic is encrypted, protocol, stickiness, connection draining, request timeout period, health

### multi-site routing

Configuration more than 1 web app on the same gateway instance. Register multiple CNAMES with the name of each site, App Gateway uses separate listeners.


### backend pool

If you’re using Azure App Service to host the back-end application, you don’t need to install any certificates in Application Gateway to connect to the back-end pool. 

### web application firewall

Optional component: checks for requests based on OWASP Core Rule Set (CRS). CRS 3.2, 3.1, 3.0, 2.2.9.
...
For devices you can use Device Writeback 
...
Group kinds:
  * Security Groups
  * Microsoft 265 groups
  * Dynamic group <— membership generated by formula (attribute values that match the filter)
...
Licenses can be assigned to a group (not just to individual users). Must have a license for each unique user.
...
Allows user to have local account AND Entra account attached for org access. (Can also hook up into Microsoft Intune)

Also: Entra joined devices
